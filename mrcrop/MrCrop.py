...
d = os.path.dirname(sys.modules["mrcrop"].__file__)
...
def crop_positions(imgh, imgw, x, y, w, h,
    ...
    xpad = (width_crop - w) / 1.8
    ypad = (height_crop - h) / 1.8
...
def updateTrasparancy(fileToProcess):
    ...
    if image_mode == "RGBA":
        ...
        processed.paste(new, (0, 0), new)
        image = processed
    return image

def main(input_d,output_d,reject_d,unknown_d,fheight=500,fwidth=500,facePercent=50,padUp=False,padDown=False,padLeft=False,padRight=False):
    ...
    output_count = 0
    unknown_count = 0
    input_files = [ os.path.join(input_d, f) for f in os.listdir(input_d) if any(f.endswith(t) for t in INPUT_FILETYPES)]
    if output_d is None:
        output_d = input_d
    ...
    for input_filename in input_files:
        ...
        unknown_filename = os.path.join(unknown_d, basename)
        try:
            # Attempt the crop
            temp = np.array(updateTrasparancy(input_filename))
            input_img = temp[:, :, ::-1].copy()
            if len(input_img) > 0:
                image = crop(input_img, fheight,fwidth,facePercent,padUp,padDown,padLeft,padRight)
                ...
                if isinstance(image, type(None)):
                    ...
                    output_count += 1
            else: shutil.copy(input_filename, reject_filename)
        except:
            shutil.copy(input_filename, unknown_filename)
            unknown_count +=1

    print('{} input files, {} faces cropped, {} rejected and {} failed to process'.format(input_count, output_count, reject_count,unknown_count))
...
def parse_args(args):
    help_d = {
            'desc': 'Automatically crops profile pictures/thumbs for websites from batches of pictures',
            'input': '''Folder where images to crop are located. Default:
                     current working directory''',
            'output': '''Folder where cropped images will be copied to.
            ...
            'unknown': 'Directory to put unknown images which failed to load',
            }

    parser = argparse.ArgumentParser(description=help_d['desc'])
    parser.add_argument('-I', '--input', default='.', type=input_path, help=help_d['input'])
    parser.add_argument('-O', '--output', '-p', '--path', type=output_path, default=None, help=help_d['output'])
    parser.add_argument('-R', '--reject', type=output_path, default=None, help=help_d['reject'])
    parser.add_argument('-U', '--unknown', type=output_path, default=None, help=help_d['unknown'])
